 ...
 */
void canIsrTxHandler(void)                                          //CAN 메시지를 전송하는 인터럽트 루틴을 처리하는 데 사용
{
...
void initMultican(void)
{
     ...
     * ==========================================================================================
     */
    IfxMultican_Can_initModuleConfig(&g_multican.canConfig, &MODULE_CAN);                                   //CAN 모듈의 구성을 초기화하는 역할

    g_multican.canConfig.nodePointer[TX_INTERRUPT_SRC_ID].priority = ISR_PRIORITY_CAN_TX;                   //CAN 모듈 구성에서 송신 인터럽트 노드의 우선순위 설정
    g_multican.canConfig.nodePointer[RX_INTERRUPT_SRC_ID].priority = ISR_PRIORITY_CAN_RX;                   //CAN 모듈 구성에서 수신 인터럽트 노드의 우선순위를 설정
     ...
     * ==========================================================================================
     */
    IfxMultican_Can_Node_initConfig(&g_multican.canNodeConfig, &g_multican.can);                               //CAN 노드 구성을 초기화
    ...
    IfxPort_setPinModeOutput(STB, IfxPort_OutputMode_pushPull, IfxPort_OutputIdx_general);  //STB핀 아웃풋으로 설정
    /* Set STB Pin of CAN chip (low-level active) */
    IfxPort_setPinLow(STB);                     //STB핀 LOW로 설정
}
...
{
   IfxPort_setPinHigh(g_led.led1.port, g_led.led1.pinIndex);                        //LED1 pin high 설정
   waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, WAIT_TIME_10ms));    /* Wait 10 milliseconds            */
   IfxPort_setPinLow(g_led.led1.port, g_led.led1.pinIndex);                         //LED1 pin low 설정
}
void blinkLED2(void)
{
    IfxPort_setPinHigh(g_led.led2.port, g_led.led2.pinIndex);                        /* Toggle the state of the LED2      */
