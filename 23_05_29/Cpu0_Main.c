...
int core0_main(void)
{
    ...
    initMultican();                                     // CAN 초기화
    initLed();                                          //LED 초기화
    //transmitCanMessage();

    while(1)
    {
        transmitCanMessage();                                   //CAN메세지 전송 함수
        waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, WAIT_TIME_50ms));    /* Wait 50 milliseconds//WAIT_TIME_xxMS만큼 대기*/
    }
    return (1);
}
